...
import { Shield, Lock, ChevronRight, User, AlertCircle, Key, Unlock } from 'lucide-react';
...
const LoginScreen: React.FC<LoginScreenProps> = ({ staff, onLogin, onCreateAdmin }) => {
  ...
  const employees = staff.filter(s => s.role !== 'Manager');
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (pin === selectedUser.pin) {
      ...
      if (selectedUser.role === 'Manager') appRole = 'Manager';
      if (selectedUser.role === 'Maintenance') appRole = 'Contractor';
        ...
        {staff.length === 0 ? (
          <div className="space-y-6">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-white">Setup Admin Account</h2>
              <p className="text-slate-400 text-sm mt-2">No users found. Create your administrator profile to get started.</p>
            </div>
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-800/50 border border-slate-600 focus:border-emerald-500 rounded-lg py-3 px-4 text-white placeholder-slate-500 outline-none transition-all"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">CREATE PIN (4 DIGITS)</label>
                <input
                  type="text"
                  inputMode="numeric"
                  required
                  ...
                  className="w-full bg-slate-800/50 border border-slate-600 focus:border-emerald-500 rounded-lg py-3 px-4 text-white placeholder-slate-500 outline-none font-mono tracking-widest transition-all"
                />
              </div>
              <button
                type="submit"
                disabled={!newAdminName || newAdminPin.length < 4}
                className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition-all shadow-lg shadow-emerald-900/20 mt-2"
              >
                Create & Login
              </button>
            </form>
                        ...
                        {managers.map(user => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-purple-500/20 text-purple-400 rounded-full flex items-center justify-center font-bold">
                                {user.name.charAt(0)}
                              </div>
